...
class FlashCosCore:
    ...
    def search(
        ...
        if search_type == 0:
            ...
            if reverse:
                match_table_q = match_table_q / (np.sqrt(np.sum(np.square(match_table_q), axis=0)) + 1e-6)
            ...
            if reverse:
                match_table_q_rev = match_table_q / (np.sqrt(np.sum(np.square(match_table_q), axis=0)) + 1e-6)
                # calculate similarity
    ...
    def search_hybrid(self, precursor_mz=None, peaks=None, ms2_tolerance_in_da=0.02, reverse=False):
        ...
        if reverse:
            match_table_q_all_rev = match_table_q_all / (np.sqrt(np.sum(np.square(match_table_q_all), axis=0)) + 1e-6)
            # calculate similarity
            similarity_arr = np.sum(match_table_q_all_rev * match_table_r_all, axis=0)

        else:
            match_table_q_all = match_table_q_all / (np.sqrt(np.sum(np.square(peaks[:, 1]), axis=0)) + 1e-6)
            # calculate similarity
...
class FlashCos:
    ...
    def search(self,
        ...
        # do not transform/normalize the intensity here, as the intensity will be normalized after alignment
...
if __name__ == "__main__":
    ...
    spectral_library = [{
        ...
        "id": "Demo spectrum 3",
        "precursor_mz": 250.0,
        "peaks": np.array([[100.0, 100], [101.0, 5], [202.0, 20], [205.0, 20]], dtype=np.float32),
        "XXX": "YYY",
    }, {
        "precursor_mz": 350.0,
    ...
    search_result = search_eng.search(
        precursor_mz=250.0,
        peaks=np.array([[100.0, 100], [101.0, 30], [202.0, 4], [203.0, 3], [204.0, 10], [205.0, 0]]),
