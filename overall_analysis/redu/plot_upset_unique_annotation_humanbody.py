...
def load_data():
    """
    Load unique annotation data for human body parts
    """
    print('Loading unique annotation data...')
    # load data
    with open('overall_analysis/redu/data/all_unique_annotation_human_bodypart.pkl', 'rb') as f:
        all = pickle.load(f)
...
def create_upset_plot(upset_data, binary_df, figsize=(8.8, 3.5), min_subset_size=5):
    ...
    upset = UpSet(
        ...
        totals_plot_elements=4,        # Show totals bars
    ...
    # Save the plot
    plt.tight_layout()
    plt.savefig('overall_analysis/redu/plots/upset_human_bodypart_annotations.svg', 
...
def plot_body_part_distribution(binary_df, figsize=(10, 6)):
    ...
    plt.title('Distribution of Unique Annotations Across Human Body Parts', fontsize=14)
...
def main():
    ...
    # Create UpSet plot
    fig = create_upset_plot(upset_data, binary_df, figsize=(8.6, 2.15), min_subset_size=100)
